from industry.leader_get_data_new import *
...
class LeaderMaker(object):
    ...
    def __init__(self, cal_day, indicator,
        ...
        self.tot_industry = self.all.groupby(self.sw).sum()
    ...
    def add_weight_in_industry_sw3(self):
        self.tot_industry[self.sw] = self.tot_industry.index
        self.tot_industry = self.tot_industry.reset_index(drop=True)
        self.all = self.all.merge(self.tot_industry, on=self.sw, how='inner')
        ...
        if self.industry_weight == 'market':
            ...
            self.zz500_weight = self.zz500_weight[
                ...
                list(self.zz500_weight['date'][self.zz500_weight['date']
                                               >= self.cal_day])[0]].reset_index(drop=True)
            # 与成分股权重表匹配最接近的日期
    ...
    def add_rank(self):
        ...
        self.all = self.all.merge(
            self.tot_industry[[self.sw, 'rank']], on=self.sw,
            how='inner').sort_values(by='rank').reset_index(drop=True)

    # 追加行业个股数量字段
    def add_industry_num(self):
        count_industry = self.all.groupby(self.sw).count()
        count_industry['indu_num'] = count_industry['code']
        count_industry[self.sw] = count_industry.index
        count_industry.index.name = self.sw + 's'
        self.all = self.all.merge(
            count_industry[[self.sw, 'indu_num']],
            on=self.sw, how='inner')
    ...
    def filter(self):
        ...
        delete_list = []
        indu_list = list(self.all[self.sw].unique())
        for indu in indu_list:
            one_indu = self.all[self.all[self.sw] == indu].copy()
        ...
        for indu in indu_list:
            one_indu = top5_industry[top5_industry[self.sw] == indu].copy()
            one_indu['global_index'] = one_indu.index

            # 如果10%不足1只，就保留1只市值最大的
            if one_indu['ten_percent'].iloc[0] < 1:
                one_indu = one_indu.sort_values(by='weight_in_industry',
                                                ascending=False)
                delete_list = delete_list + list(one_indu['global_index'].iloc[1:])

            # 如果10%大于1只
            if 1 <= one_indu['ten_percent'].iloc[0] < 5:
                one_indu = one_indu.sort_values(by='weight_in_industry',
                                                ascending=False)
                one_indu_withoutbig = one_indu[one_indu['weight_in_industry']
                                               < 0.35].reset_index(drop=True)
                if len(one_indu_withoutbig) > 0:

                    # 如果有寡头，这里为除了寡头的要保留的股票数量，如果没有寡头，就保留前10%只
...
if __name__ == "__main__":
    ...
    for changeday in change_days:
        ...
        weighted_result.to_excel(
            'C:/Users/Lenovo/PycharmProjects/my_project/frjj_codes/' + 'sw3_' + filename + '.xls')

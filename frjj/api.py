...
def get_data_crosssection(user, passwd, data_type, date, database="stock_cn"):
    # TODO:指数成分更新完后删除下两行
    # TODO：行业划分数据2019年后时间频率没有规律
    if date > '2019-07-02' and data_type != 'stock_cn':
        date = '2019-07-02'
    if date > '2018-12-31' and data_type == 'industry':
        date = '2019-03-08'
    myclient = pymongo.MongoClient("192.168.10.33", 27017)
    myclient.admin.authenticate(user, passwd)
...
def get_data_panel_backwards(user, passwd, code_list,
    ...
    next_date_str = forwardchangeday2(next_date_str)
...
def get_sw_info(code_list, begin_date, end_date):
    if len(begin_date) == 8:
        begin_date = begin_date[0:4] + '-' + begin_date[4:6] + '-' + begin_date[6:8]
    else:
        pass
    if len(end_date) == 8:
        ...
        pass
    myclient = pymongo.MongoClient("192.168.10.17", 27017)
    myclient.admin.authenticate("huangwei", "huangwei")
    mydb = myclient["stock_cn"]
    mycol = mydb['industry_sw']
    myquery = {"$and": [{"date": {"$gte": begin_date, "$lte": end_date}}, {"code": {"$in": code_list}}]}
    mydoc = mycol.find(myquery)
    df = pd.DataFrame(list(mydoc))
    del df['_id']
